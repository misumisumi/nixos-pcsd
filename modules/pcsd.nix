...
let
  ...
  indentShellLines =
    n: text:
    let
      indentLine = line: if line == "" then "" else "${replicate n " "}${trim line}";
      lines = splitString "\n" text;
    in
    concatMapStringsSep "\n" indentLine lines;
in
{
  disabledModules = [ "services/cluster/pacemaker/default.nix" ];
  imports = [ ./pacemaker.nix ];
  ...
  config =
    ...
    mkIf cfg.enable {
      ...
      systemd.services =
        let
          # More resource funcs
          mkResource = resource: (resourceTypeInfo resource).createCmd;
